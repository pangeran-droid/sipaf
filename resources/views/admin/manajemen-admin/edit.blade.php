@extends('layouts.admin', ['title' => 'Edit Admin - SIPAF Admin'])

@section('content')
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2 fw-bold">Edit Akun Admin: {{ $admin->name }}</h1>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                ...
                <form action="{{ route('admin.manajemen-admin.update', $admin->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="password" class="form-label fw-bold">Password Baru <span class="text-muted fw-normal small">(Kosongkan jika tidak ingin mengubah password)</span></label>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="role" id="role" class="form-select" required onchange="toggleJurusan(this.value)">
                            <option value="admin" {{ old('role', $admin->role) == 'admin' ? 'selected' : '' }}>Admin Jurusan</option>
                    ...
                    <div class="mb-3" id="jurusanWrapper">
                        ...
                        <select name="jurusan_id" id="jurusan_id" class="form-select">
                            ...
                            @foreach($jurusans as $jurusan)
                                <option value="{{ $jurusan->id }}" {{ old('jurusan_id', $admin->jurusan_id) == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama_jurusan }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="d-grid">
                        <button type="submit" class="btn btn-primary">Perbarui Akun Admin</button>
                    </div>
                </form>
            </div>
        </div>
